...
fn main() -> Result<()> {
    ...
    let device = std::fs::File::open(&args[1])?;
    ...
    if status == VA_STATUS_SUCCESS as i32 {
        ...
        bail!(format!("Failed to initialize VA-API on DRM display: {:?}", device));
    }
    ...
    profiles.retain(|&p| {
        ...
        for e in entrypoints.iter() {
            let mut attrib_list = vec![
                VAConfigAttrib{type_: VAConfigAttribEncMaxRefFrames, value: VA_ATTRIB_NOT_SUPPORTED},
                VAConfigAttrib{type_: VAConfigAttribMaxPictureWidth, value: VA_ATTRIB_NOT_SUPPORTED},
                VAConfigAttrib{type_: VAConfigAttribMaxPictureHeight, value: VA_ATTRIB_NOT_SUPPORTED},
                VAConfigAttrib{type_: VAConfigAttribEncQualityRange, value: VA_ATTRIB_NOT_SUPPORTED},
                VAConfigAttrib{type_: VAConfigAttribEncPackedHeaders, value: VA_ATTRIB_NOT_SUPPORTED},
                VAConfigAttrib{type_: VAConfigAttribEncQuantization, value: VA_ATTRIB_NOT_SUPPORTED},
                VAConfigAttrib{type_: VAConfigAttribEncMacroblockInfo, value: VA_ATTRIB_NOT_SUPPORTED},
            ];
            status = unsafe { vaGetConfigAttributes(display,
                                          p,
                                          *e,
            ...
            let status = unsafe { vaCreateConfig(display, p, *e, std::ptr::null_mut(), 0, &mut config) };
            if status != VA_STATUS_SUCCESS as i32 || config == 0 as VAConfigID {
                return false;
            }

            let mut num_surf_attribs = 0;
            let status = unsafe { vaQuerySurfaceAttributes(display, config, std::ptr::null_mut(), &mut num_surf_attribs) };
            if status != VA_STATUS_SUCCESS as i32 {
                println!("Failed to query surface attributes size");
                return false
            }
            let mut surf_attribs = unsafe { vec![ std::mem::zeroed::<VASurfaceAttrib>(); num_surf_attribs as usize] };
            let status = unsafe { vaQuerySurfaceAttributes(display,
                                                          config,
            ...
            for s in surf_attribs.iter() {
                if s.type_ == VASurfaceAttribPixelFormat {
                    println!("\t\t{:?}:{:?}", s.type_, fourcc(&s.value.value.bindgen_union_field));
                } else if s.type_ == VASurfaceAttribMemoryType {
                    println!("\t\t{:?}:0b{:b}", s.type_, s.value.value.bindgen_union_field as u32);
                } else {
